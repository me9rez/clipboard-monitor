...
describe('ClipboardMonitor', () => {
    ...
    afterEach(async () => {
        ...
        if (monitor?.child && !monitor.child.killed) {
            await new Promise(r => monitor.child.once('exit', r));
        }
    });

    it('starts child process and receives messages via stdout', async () => {
        const fakeScript = join(__dirname, 'fake-clipboard-monitor.js');
    ...
    it('handleMessage dispatches text correctly', () => {
        ...
        expect(logs.some(args => args[0]?.includes?.('剪贴板文本') && args.includes('test text'))).toBe(true);
    });
    ...
    it('handleMessage dispatches files correctly', () => {
        ...
        console.log = originalLog;
        expect(logs.some(args => args[0]?.includes?.('剪贴板文件列表'))).toBe(true);
    });
    ...
    it('handleMessage dispatches error correctly', () => {
        ...
        expect(errors.some(args => args[0]?.includes?.('C# 内部错误') && args.includes('boom'))).toBe(true);
    });
    ...
    it('processAndCleanImage reads and deletes temp file', async () => {
        ...
        expect(logs.some(args => args[0]?.includes?.('读取完毕'))).toBe(true);
    });
    ...
    it('processAndCleanImage skips when file does not exist', async () => {
        ...
        console.log = originalLog;
        expect(logs.some(args => args[0]?.includes?.('读取完毕'))).toBe(false);
    });
});
